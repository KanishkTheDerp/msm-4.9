				 ...
				 block_t blkaddr, int flags)
{
	...
	bio = f2fs_grab_read_bio(inode, blkaddr, 1, 0, page->index, flags);
	if (IS_ERR(bio))
		return PTR_ERR(bio);
...
zero_out:
		if (f2fs_verity_file(inode) && !fsverity_verify_page(page)) {
			ret = -EFAULT;
			goto out;
		}
		zero_user_segment(page, 0, PAGE_SIZE);
...
submit_and_realloc:
	...
	if (bio == NULL) {
		bio = f2fs_grab_read_bio(inode, block_nr, nr_pages,
				is_readahead ? REQ_RAHEAD : 0, page->index, 0);
		if (IS_ERR(bio)) {
			ret = PTR_ERR(bio);
			bio = NULL;
			goto out;
